// features/emails/utils/renderEmail.ts
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";

// import { formatSalesOrderNumber } from "@/shared/utils/formatOrderNumbers";
...
function renderTemplate(contentFile: string, args: RenderTemplateArgs) {
  const { firstName = "there", url, offerExpiration } = args;
...
export function renderAccountCreatedEmail({ firstName, url }: { firstName: string; url: string }) {
  return renderTemplate("accountCreated.raw.html", { firstName, url });
}

export function renderResetPasswordEmail({ firstName, url }: { firstName: string; url: string }) {
  return renderTemplate("resetPassword.raw.html", { firstName, url });
}

export function renderVerifyEmail({ firstName, url }: { firstName: string; url: string }) {
  return renderTemplate("verifyEmail.raw.html", { firstName, url });
}

export function renderChangeEmail({ firstName, url }: { firstName: string; url: string }) {
  return renderTemplate("changeEmail.raw.html", { firstName, url });
}

export function renderCreateAccountEmail({ firstName, url }: { firstName: string; url: string }) {
  return renderTemplate("createAccount.raw.html", { firstName, url });
}

export function renderPurchaseOrderPlacedEmail({ firstName, url }: { firstName: string; url: string }) {
  return renderTemplate("purchaseOrderPlaced.raw.html", { firstName, url });
}

export function renderSalesOrderPlacedEmail({ firstName, url }: { firstName: string; url: string }) {
...
export function renderOfferSentEmail({
  ...
  return renderTemplate("offerSent.raw.html", { firstName, url, offerExpiration });
}

export function renderOfferAcceptedEmail({ firstName, url }: { firstName: string; url: string }) {
  return renderTemplate("offerAccepted.raw.html", { firstName, url });
}

// export function renderSalesOrderToSupplierEmail(args: {
//   firstName: string;
//   url: string;
//   order: any;
//   spots: Array<{ type: string; ask_spot: number }>;
// }) {
//   const { firstName, url, order, spots } = args;
